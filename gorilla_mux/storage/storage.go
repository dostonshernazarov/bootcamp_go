...
func CreateUser(user *modelMux.UserMux) (*modelMux.UserMux, error) {
	db, err := connect()
	if err != nil {
		return nil, err
	}
...
func GetUser(userID string) (*modelMux.UserMux, error) {
	db, err := connect()
	if err != nil {
		return nil, err
	}
...
func GetAll(page, limit int) (users []*modelMux.UserMux, err error) {
	db, err := connect()
	if err != nil {
		return nil, err
	}
...
func UpdateUserById(user *modelMux.UserMux) (*modelMux.UserMux, error) {
	db, err := connect()
	if err != nil {
		return nil, err
	}
